/**
 * deriveChromeTokens derives owl's contrast-heavy "chrome" token set — the
...
// The opinionated app-chrome token set owlChromeMapping maps onto its CSS
...
function pickReadableMuted(
  ...
  if (mutedCandidate == null || mutedCandidate === "") {
    return undefined;
  }
...
export function deriveChromeTokens(theme: ThemeLike): ChromeTokens | undefined {
  ...
  if (cached !== undefined || cache.has(theme)) {
    return cached;
  }
