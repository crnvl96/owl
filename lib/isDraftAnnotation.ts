...
// is on a file item (line only). The metadata shape is identical between
// the two, so the predicate just narrows on `metadata.kind` and the return
// type is a union to match the input.
